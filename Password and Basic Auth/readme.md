...
The server sends a 401 and the browser pops up the built-in login prompt. User enters correct mail and password and POSTGRESQL validates.

On the other hand, if a users has been authenticated using my own system, the the LUA file will detect that, and the prompt for the apache basic auth will NOT popup.
...
________________  OLD STUFF from before 2026 ________________
